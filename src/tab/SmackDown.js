...
class SmackDown extends Component {
    constructor() {
        super()
    ...
    componentDidMount() {
        ...
        this.props.navigation.addListener('didFocus', (route) => { 
            ...
            AppEventsLogger.logEvent(`SMACKDOWN_click`)
        });
        if(!config.ios){
    ...
    onPostChange = (position) => {
        ...
        if(post_list.length-position < 5 && loading_more == false){
            loading_more = true
            Wrestlefeed.fetchPostData("SMACKDOWN", last_id, user_data.ID).then((post) => {
    ...
    onRefreshPost = () => {
        ...
        if(post_list.length != 0){
            ...
            Wrestlefeed.fetchNewPost("SMACKDOWN", post_list, user_data).then((resRefreshPostList) => {
                if(resRefreshPostList.length != 0){
                    this.setState({ post_list: resRefreshPostList, refresh_load: false }, () => {
                        this.setToLatest(3, true, resRefreshPostList[0].id)
                        setTimeout(() => {
                            this.viewPager.current.setPage(0);
                        }, 300)
                    })
