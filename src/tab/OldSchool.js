...
class OldSchool extends Component {
    ...
    componentDidMount() {
        ...
        this.props.navigation.addListener('didFocus', (route) => { 
            AppEventsLogger.logEvent(`OLD SCHOOL_click`)
            this.setToLatest(7, false, 0)
        });
        if(!config.ios){
    ...
    onPostChange = (position) => {
        ...
        if(post_list.length-position < 7 && loading_more == false){
            loading_more = true
            Wrestlefeed.fetchPostData("OLD SCHOOL", last_id, user_data.ID).then((post) => {
    ...
    onRefreshPost = () => {
        ...
        if(post_list.length != 0){
            ...
            Wrestlefeed.fetchNewPost("OLD SCHOOL", post_list, user_data).then((resRefreshPostList) => {
                if(resRefreshPostList.length != 0){
                    this.setState({ post_list: resRefreshPostList, refresh_load: false }, () => {
                        this.setToLatest(7, true, resRefreshPostList[0].id)
                        setTimeout(() => {
                            this.viewPager.current.setPage(0);
                        }, 300)
                    })
...
export default connect(mapStateToProps, { updateDarkMode, pushTabData, refreshAds })(withNavigationFocus(OldSchool));
