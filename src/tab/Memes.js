...
class Memes extends Component {
  ...
  componentDidMount() {
    ...
    this.props.navigation.addListener('didFocus', (route) => {
      ...
      AppEventsLogger.logEvent(`MEMES_click`)
    });
  ...
  onPostChange = (position) => {
    ...
    if (post_list.length - position < 5 && loading_more == false) {
      loading_more = true
      Wrestlefeed.fetchPostData("MEMES", last_id, user_data.ID).then((post) => {
  ...
  onRefreshPost = () => {
    ...
    if (post_list.length != 0) {
      ...
      Wrestlefeed.fetchNewPost("MEMES", post_list, user_data).then((resRefreshPostList) => {
        if (resRefreshPostList.length != 0) {
          this.setState({ post_list: resRefreshPostList, refresh_load: false }, () => {
            this.setToLatest(8, true, resRefreshPostList[0].id)
            setTimeout(() => {
              this.viewPager.current.setPage(0);
            }, 300)
          })
...
export default connect(mapStateToProps, { updateDarkMode, pushTabData, refreshAds })(withNavigationFocus(Memes));